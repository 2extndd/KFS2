...
def build_search_url(topic_info):
    ...
    # Добавляем поисковый запрос и категорию
    if query_params.get("q"):
        search_query = query_params["q"].replace(" ", "%20")
        category_id = query_params.get("categoryId", "")
        
        if category_id:
            # Используем первую категорию если указано несколько через запятую
            first_category = category_id.split(",")[0]
            base_url += f"/s-{search_query}/k0c{first_category}"
        else:
        ...
        if category_id:
            first_category = category_id.split(",")[0]
            base_url += f"/s-anzeigen/k0c{first_category}"
        else:
